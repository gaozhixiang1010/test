				...
				if (ref && refUrl) {
					function _onchange(event)
					{
						...
						if(targetType =='navTab')
							$ref =  $("#"+ref,navTab.getCurrentPanel());
						else if(targetType == 'dialog')
							$ref =  $("#"+ref,$.pdialog.getCurrent());
						else
						 	$ref =  $("#"+ref);
						...
						$.ajax({
							...
							type:'POST', dataType:"json", url:$this.attr("refUrl").replace("{value}", encodeURIComponent($this.attr("value"))), cache: false,
							data:{},
							success: function(json){
								if (!json) return;
								var html = '';
								var html2 = '';
								var $kk = $('button[data-id='+ref+']');
								var $refCombox = $('button[data-id='+ref+']').parents('div.bootstrap-select:first');
								var $hh = $refCombox.find('ul[role=menu]:first');
								for(var i=0;i<json.length;i++)
								{
									if(json[i].hidden)
									{
										html += '<option value="'+json[i].id+'" style="display:none">' + json[i].value + '</option>';
									}
									else 
										html += '<option value="'+json[i].id+'">' + json[i].value + '</option>';
									if(i == 0)
									{
										$kk.attr("title",json[i].value);
										$kk.find('span.filter-option:first').text(json[i].value);
										if(json[i].hidden)
										{
											html2 += '<li rel="'+i+'" class="selected" style="display:none">';
										}
										else
											html2 += '<li rel="'+i+'" class="selected">';
									}
									else
									{
										if(json[i].hidden)
										{
											html2 += '<li rel="'+i+'" class style = "display:none">';
										}
										else
											html2 += '<li rel="'+i+'" class style>';
									}
										
									html2 +='<a tabindex="0" class="" style=""><span class="text">'+json[i].value+'</span><i class="glyphicon glyphicon-ok icon-ok check-mark"></i></a></li>';
								}	
								$ref.html(html);
								$hh.html(html2);
								$("#alertBackground").hide();
								//$ref.trigger("change").relevance();
							},
							error: DWZ.ajaxError
						});
					}
